...
public:
	...
	bool generateHull(vector<Triangle>& triangles, vector<NeighborList>& neighborLists);
	bool isInside(int row, int column, int depth);
	const vector<unsigned int>& getIdentifiedNeighbors() {return m_distinctGrains;}
private:
	...
	inline void	insertDistinctInteger(unsigned int intger,NeighborList& nieghbors, int& elem_count) const;
	void recordNewGrains(NeighborList& grainIDs);
	int  insertNeighborList(NeighborList& neighborList, vector<NeighborList>& neighborLists);
