...
#pragma omp critical
	{
		...
		m_inputDistance = new DimensionalBufferReal(xmin, ymin, zmin, xmax,
				ymax, zmax);
		m_outputDistance = new DimensionalBufferReal(xmin, ymin, zmin, xmax,
				ymax, zmax);
...
//#endif
	...
	void LSbox::calculateMagneticEnergy() {
		...
		double rotMatrix[9] = { cos(p1) * cos(p2) - sin(p1) * sin(p2) * cos(t),
				sin(p1) * cos(p2) + cos(p1) * sin(p2) * cos(t), sin(p2)
						* sin(t), -cos(p1) * sin(p2)
						- sin(p1) * cos(p2) * cos(t), -sin(p1) * sin(p2)
				...
				for (int j = m_inputDistance->getMinX();
					...
					if (i < min || i > max || j < min || j > max || k < min
						...
						m_inputDistance->setValueAt(i, j, k,
							...
							== m_grainHandler->IDField->getValueAt(i, j, k))
						m_inputDistance->setValueAt(i, j, k,
								m_grainHandler->get_h());
					else
						m_inputDistance->setValueAt(i, j, k,
				...
				for (int i = intersec_ymin; i < intersec_ymax; i++)
					for (int j = intersec_xmin; j < intersec_xmax; j++) {
						...
						if (abs(val) < m_grainHandler->delta) {
							...
							IDChunkMinimal grain = m_IDLocal.getValueAt(i, j,
									k);
...
// Comparison + Helperfunctions
	...
	void LSbox::executeSetComparison() {
		...
		for (int k = m_outputDistance->getMinZ();
				k < m_outputDistance->getMaxZ(); k++) {
	...
	void LSbox::executeComparison() {
		...
		for (unsigned int neighs = 0; neighs < m_secondOrderNeighbours.size();
			...
			int x_min_new, x_max_new, y_min_new, y_max_new, z_min_new,
					z_max_new;

			if (m_inputDistance->getMinX()
					< neighbor->m_inputDistance->getMinX())
				x_min_new = neighbor->m_inputDistance->getMinX();
			else
				x_min_new = m_inputDistance->getMinX();

			if (m_inputDistance->getMaxX()
					> neighbor->m_inputDistance->getMaxX())
				x_max_new = neighbor->m_inputDistance->getMaxX();
			else
				x_max_new = m_inputDistance->getMaxX();

			if (m_inputDistance->getMinY()
					< neighbor->m_inputDistance->getMinY())
				y_min_new = neighbor->m_inputDistance->getMinY();
			else
				y_min_new = m_inputDistance->getMinY();

			if (m_inputDistance->getMaxY()
					> neighbor->m_inputDistance->getMaxY())
				y_max_new = neighbor->m_inputDistance->getMaxY();
			else
				y_max_new = m_inputDistance->getMaxY();

			if (m_inputDistance->getMinZ()
					< neighbor->m_inputDistance->getMinZ())
				z_min_new = neighbor->m_inputDistance->getMinZ();
			else
				z_min_new = m_inputDistance->getMinZ();

			if (m_inputDistance->getMaxZ()
					> neighbor->m_inputDistance->getMaxZ())
			...
			for (int k = z_min_new; k < z_max_new; k++) {
				for (int i = y_min_new; i < y_max_new; i++) {
					for (int j = x_min_new; j < x_max_new; j++) {
						double dist = neighbor->getDistanceFromInputBuff(i, j,
								k);
...
//  Redistancing
	...
	void LSbox::executeRedistancing() {
		...
		for (int k = intersec_zmin; k < m_outputDistance->getMaxZ() - 1; k++) {
			for (int i = intersec_ymin; i < m_outputDistance->getMaxY(); i++) {
				for (int j = intersec_xmin; j < m_outputDistance->getMaxX();
						j++) {
					// x-direction forward
...
// plot the box and all its properties
		...
		for (int k = m_inputDistance->getMinZ(); k < m_inputDistance->getMaxZ();
			...
			for (int i = m_inputDistance->getMinY();
				...
				for (int j = m_inputDistance->getMinX();
					...
					ro = 1 / (xr - xl)
							* ((xr - pointx)
									* m_outputDistance->getValueAt(yo, xl, zu)
									+ (pointx - xl)
											* m_outputDistance->getValueAt(yo,
													xr, zu));
					ru = 1 / (xr - xl)
							* ((xr - pointx)
									* m_outputDistance->getValueAt(yu, xl, zu)
									+ (pointx - xl)
											* m_outputDistance->getValueAt(yu,
													xr, zu));
					...
					double trilinearInterpolation = (zo - pointz)
							* newDistVal_zu + (pointz - zu) * newDistVal_zo;
	...
	vector<double> LSbox::linearRegression(vector<SPoint>& points2D) {
		...
		linearRegression.push_back(
				meanY - ((covarianceXY / varianceX) * meanX));
	...
	void LSbox::outputMemoryUsage(ofstream& output) {
		...
		output << m_inputDistance->getMaxX() - m_inputDistance->getMinX() << " "
				...
				<< " "
				<< m_inputDistance->getMaxZ() - m_inputDistance->getMinZ()
				<< endl;
