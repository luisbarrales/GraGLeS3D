...
LSbox::LSbox() :
	exist(false), quaternion(NULL), inputDistance(NULL), outputDistance(NULL),
			local_weights(NULL) {
}
...
LSbox::LSbox(int aID, vector<double>& vertices, grainhdl* owner) :
	id(aID), nvertices(0), handler(owner) {
	...
	if (Settings::UseTexture) {
		double newOri[3];
		(*(handler->mymath)).newOrientationFromReference(handler->bunge,
				handler->deviation, newOri);
	...
	ymax += 2 * grid_blowup;
	zmax += 2 * grid_blowup;
	int bound = handler->get_ngridpoints() - 1;
	if (xmin < 0)
		xmin = 0;
	if (ymin < 0)
		ymin = 0;
	...
	inputDistance = new DimensionalBufferReal(xmin, ymin, xmax, ymax, zmin,
			zmax);
	outputDistance = new DimensionalBufferReal(xmin, ymin, xmax, ymax, zmin,
			zmax);
...
LSbox::LSbox(int id, int nvertices, double* vertices, double q1, double q2,
	...
	id(id), nvertices(nvertices), handler(owner) {
	//	exist = true;
	//	quaternion = new double[4];
	//	quaternion[0]=q1 ;
	//	quaternion[1]=q2 ;
...
bool LSbox::isNeighbour(LSbox* candidate) {
	...
	for (it = grainCharacteristics.begin(); it != grainCharacteristics.end(); it++) {
		if ((*it).directNeighbour == candidate)
			return true;
	}
	return false;
}
...
double LSbox::mis_ori(LSbox* grain_2) {
	...
	return (*(handler->mymath)).misorientationCubicQxQ(quaternion[0],
			quaternion[1], quaternion[2], quaternion[3],
			grain_2->quaternion[0], grain_2->quaternion[1],
			grain_2->quaternion[2], grain_2->quaternion[3]);
}
...
void LSbox::conv_generator(fftwp_complex *fftTemp, fftwp_plan fftplan1,
		fftwp_plan fftplan2) {
		...
		for (int k = 0; k < n; k++) {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < n2; j++) {
					...
					G
							= exp(
									-(static_cast<double> (i2 * i2 + j2 * j2
											+ k2 * k2)) * 8.0 * dt * PI * PI
											* PI) / nsq;
...
void LSbox::distancefunction() {
	cout << "compute Distancefuntion for box " << id << endl;
		...
		for (int i = inputDistance->getMinY(); i < inputDistance->getMaxY(); i++) {
			for (int j = inputDistance->getMinX(); j < inputDistance->getMaxX(); j++) {
				if (outOfDomain(i, j, k))
					inputDistance->setValueAt(i, j, k, -handler->get_h());
				else if (id == (int) handler->IDField->getValueAt(i, j, k)) {
					inputDistance->setValueAt(i, j, k, handler->get_h());
				} else
					inputDistance->setValueAt(i, j, k, -handler->get_h());
			}
...
void LSbox::add_n2o_2() {
	...
	for (it = grainCharacteristics.begin(); it != grainCharacteristics.end(); it++) {
		...
		for (it_ngC = (*it).directNeighbour->grainCharacteristics.begin(); it_ngC
				!= (*it).directNeighbour->grainCharacteristics.end(); it_ngC++) {
	...
	for (it_nC = neighbourCandidates.begin(); it_nC
		...
		if ((*it_nC) == handler->boundary)
			continue;
		for (it_com = neighbors_2order.begin(); it_com
	...
	for (int k = outputDistance->getMinZ(); k < outputDistance->getMaxZ(); k++) {
		for (int i = outputDistance->getMinY(); i < outputDistance->getMaxY(); i++) {
			for (int j = outputDistance->getMinX(); j
				...
				if (abs(inputDistance->getValueAt(i, j, k)) < 0.7
						* handler->delta) {
					outputDistance->setValueAt(
							i,
							j,
							k,
							0.5 * (inputDistance->getValueAt(i, j, k)
									- outputDistance->getValueAt(i, j, k)));
				} else
					outputDistance->setValueAt(i, j, k,
							inputDistance->getValueAt(i, j, k));
			}
		}
	}
}
...
void LSbox::comparison(ExpandingVector<char>& mem_pool) {
	...
	mem_pool.expand(
			(outputDistance->getMaxX() - outputDistance->getMinX())
					* (outputDistance->getMaxY() - outputDistance->getMinY())
					* (outputDistance->getMaxZ() - outputDistance->getMinZ())
					* sizeof(double));
	...
	for (it_nn = neighbors_2order.begin(); it_nn != neighbors_2order.end(); it_nn++) {
		...
		if (inputDistance->getMinX() < (**it_nn).inputDistance->getMinX())
			x_min_new = (**it_nn).inputDistance->getMinX();
		else
			x_min_new = inputDistance->getMinX();

		if (inputDistance->getMaxX() > (**it_nn).inputDistance->getMaxX())
			x_max_new = (**it_nn).inputDistance->getMaxX();
		else
			x_max_new = inputDistance->getMaxX();

		if (inputDistance->getMinY() < (**it_nn).inputDistance->getMinY())
			y_min_new = (**it_nn).inputDistance->getMinY();
		else
			y_min_new = inputDistance->getMinY();

		if (inputDistance->getMaxY() > (**it_nn).inputDistance->getMaxY())
			y_max_new = (**it_nn).inputDistance->getMaxY();
		else
			y_max_new = inputDistance->getMaxY();

		if (inputDistance->getMinZ() < (**it_nn).inputDistance->getMinZ())
			z_min_new = (**it_nn).inputDistance->getMinZ();
		else
			z_min_new = inputDistance->getMinZ();

		if (inputDistance->getMaxZ() > (**it_nn).inputDistance->getMaxZ())
			z_max_new = (**it_nn).inputDistance->getMaxZ();
		else
			z_max_new = inputDistance->getMaxZ();

		for (int k = z_min_new; k < z_max_new; k++) {
			for (int i = y_min_new; i < y_max_new; i++) {
				for (int j = x_min_new; j < x_max_new; j++) {
					if (abs(inputDistance->getValueAt(i, j, k))
							< handler->delta) {
...
bool LSbox::BoundaryIntersection() {
	int xMinBoundary = handler->get_grid_blowup()
			+ handler->getBoundaryGrainTube();
	int yMinBoundary = xMinBoundary;
	int zMinBoundary = xMinBoundary;

	int xMaxBoundary = handler->get_ngridpoints() - handler->get_grid_blowup()
			- handler->getBoundaryGrainTube();
	int yMaxBoundary = xMaxBoundary;
	int zMaxBoundary = xMaxBoundary;

	if (outputDistance->getMinX() > xMinBoundary && outputDistance->getMaxX()
			< xMaxBoundary && outputDistance->getMinY() > yMinBoundary
			&& outputDistance->getMaxY() < yMaxBoundary
...
void LSbox::boundaryCondition() {
	...
	int m = handler->get_ngridpoints();
	int distZMin, distZMax, distXMin, distXMax, distX;
	int distYMin, distYMax, distY, distZ;
	double dist;
...
void LSbox::redist_box() {
	...
	if (inputDistance->getMaxZ() < outputDistance->getMaxZ())
		...
		intersec_zmax = outputDistance->getMaxZ();
			...
			for (int j = intersec_xmin; j < outputDistance->getMaxX() - 1; j++) {
				...
				if (j < intersec_xmax - 1 && i < intersec_ymax) {
					if (inputDistance->getValueAt(i, j, k)
							* inputDistance->getValueAt(i, j + 1, k) <= 0.0) {
						// interpolate
						i_slope = (inputDistance->getValueAt(i, j + 1, k)
								- inputDistance->getValueAt(i, j, k)) / h;
						distToZero = -inputDistance->getValueAt(i, j, k)
								/ i_slope;
						if (abs(outputDistance->getValueAt(i, j, k)) > abs(
								distToZero))
							outputDistance->setValueAt(i, j, k,
									-distToZero * utils::sgn(i_slope));
					}
					candidate
							= outputDistance->getValueAt(i, j, k)
									+ (utils::sgn(
											inputDistance->getValueAt(i, j + 1,
													k)) * h);
					if (abs(candidate) < abs(
							outputDistance->getValueAt(i, j + 1, k)))
						outputDistance->setValueAt(i, j + 1, k, candidate);
				} else {
					candidate = outputDistance->getValueAt(i, j, k)
							+ (utils::sgn(
									outputDistance->getValueAt(i, j + 1, k))
									* h);
					if (abs(candidate) < abs(
		...
		for (int j = intersec_xmin; j < outputDistance->getMaxX(); j++) {
			for (int i = intersec_ymin; i < outputDistance->getMaxY() - 1; i++) {
				if (j < intersec_xmax && i < intersec_ymax - 1) {
					if (inputDistance->getValueAt(i, j, k)
							* inputDistance->getValueAt(i + 1, j, k) <= 0.0) {
						// interpolate
						i_slope = (inputDistance->getValueAt(i + 1, j, k)
								- inputDistance->getValueAt(i, j, k)) / h;
						distToZero = -inputDistance->getValueAt(i, j, k)
								/ i_slope;
						if (abs(outputDistance->getValueAt(i, j, k)) > abs(
					...
					candidate
							= outputDistance->getValueAt(i, j, k)
									+ (utils::sgn(
											inputDistance->getValueAt(i + 1, j,
													k)) * h);
					if (abs(candidate) < abs(
							outputDistance->getValueAt(i + 1, j, k)))
			...
			for (int j = intersec_xmin; j < outputDistance->getMaxX(); j++) {
				...
				if (k < intersec_zmax - 1) {
					if (inputDistance->getValueAt(i, j, k)
						...
						// interpolate
						i_slope = (inputDistance->getValueAt(i, j, k + 1)
								- inputDistance->getValueAt(i, j, k)) / h;
						distToZero = -inputDistance->getValueAt(i, j, k)
								/ i_slope;
						if (abs(outputDistance->getValueAt(i, j, k)) > abs(
					...
					candidate
							= outputDistance->getValueAt(i, j, k)
									+ (utils::sgn(
											inputDistance->getValueAt(i, j,
													k + 1)) * h);
					if (abs(candidate) < abs(
							outputDistance->getValueAt(i, j, k + 1)))
...
#pragma omp critical
	{
		makeFFTPlans(inputDistance->getRawData(), outputDistance->getRawData(),
				fftTemp, &fwdPlan, &bwdPlan);
	}
...
void LSbox::plot_box_3d(int select, string simstep) {
	stringstream filename;
	ofstream datei;
	//	cout << "boxsize: InputBuffer: xminIn, xmaxIn, yminIn, ymaxIn zminIn, zmaxIn : "
	//	<< inputDistance->getMinX() << " || " << inputDistance->getMaxX()
	//	<< " || " << inputDistance->getMinY() << " || "
	//	<< inputDistance->getMaxY() << " || " << inputDistance->getMinZ() << " || "
	//	<< inputDistance->getMaxZ()
	//	<< endl;
	//	cout << "boxsize: Outputbuffer xminIn, xmaxIn, yminIn, ymaxIn zminIn, zmaxIn : "
	//	<< outputDistance->getMinX() << " || " << outputDistance->getMaxX()
	//	<< " || " << outputDistance->getMinY() << " || "
	//	<< outputDistance->getMaxY() << " || " << outputDistance->getMinZ() << " || "
	//	<< outputDistance->getMaxZ()
	//	<< endl;
	if (select == 2) {
		...
		for (int k = inputDistance->getMinZ(); k < inputDistance->getMaxZ(); k++) {
			for (int i = inputDistance->getMinY(); i < inputDistance->getMaxY(); i++) {
				for (int j = inputDistance->getMinX(); j
					...
					//							k) )>0)
					datei << i << "  " << j << "  " << k << "  "
							<< ::std::fixed << outputDistance->getValueAt(i, j,
							k) << endl;
				}
			}
		}
	...
	if (select == 1) {
		filename << "Box3D_" << simstep << "in_T" << handler->loop << "_" << id
				<< ".gnu";
		datei.open(filename.str());
		for (int k = inputDistance->getMinZ(); k < inputDistance->getMaxZ(); k++) {
			for (int i = inputDistance->getMinY(); i < inputDistance->getMaxY(); i++) {
				for (int j = inputDistance->getMinX(); j
						< inputDistance->getMaxX(); j++) {
					//					if ((inputDistance->getValueAt(i, j,
					//												k) )>0)
					datei << i << "  " << j << "  " << k << "  "
							<< ::std::fixed << inputDistance->getValueAt(i, j,
							k) << endl;
				}
			}
		}
